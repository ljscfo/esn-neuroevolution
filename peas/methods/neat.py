...
class NEATGenotype(object):
    ...
    def mutate(self, innovations={}, global_innov=0):
        ...
        # neat-python does, so I'm copying.
            ...
            potential_conns = product(list(range(len(self.node_genes))), list(range(self.inputs, len(self.node_genes))))
    ...
    def get_network_data(self):
        ...
        cm.fill(np.nan)
        for (_, fr, to, weight, enabled) in list(self.conn_genes.values()):
            if enabled:
                cm[to, fr] = weight

        # Reorder the nodes/connections
        ff, node_types, bias, response, layer = list(zip(*self.node_genes))
        order = [i for _,i in sorted(zip(ff, list(range(len(ff)))))]
        cm = cm[:,order][order,:]
...
class NEATPopulation(SimplePopulation):
    ...
    def _status_report(self):
        ...
        print(("Best (%.5f): %s" % (self.champions[-1].stats['fitness'],self.champions[-1])))#, self.champions[-1].stats)))
